        ...
        <div className="pt-10 sm:pt-16 lg:pt-10 xl:pt-[5rem]">
          ...
          <div className="mx-5 mb-20 sm:mx-auto sm:px-12 md:px-16 lg:px-12 mt-6 sm:shadow-md sm:max-w-[28rem] md:max-w-[32rem] lg:max-w-[35rem] sm:rounded-lg py-5 lg:pt-14 lg:pb-20 sm:pt-9 sm:pb-16">
            <div className="text-center text-[#464646]">
              ...
              <h3 className="font-light mt-8 text-sm">Create an account to track your purchased items</h3>
            </div>

            <form className="mt-5">
              <div className="sm:flex sm:justify-between sm:space-x-4">
                <div className="w-full">
                  <label className="text-xs text-[#464646]">First Name</label>
                  <br />
                  <input autoComplete="off" className="border-2 focus:outline-[#056424] focus:bg-white text-[#464646] text-sm w-full mt-1 h-12 rounded-md px-2 placeholder:text-xs" type="text" placeholder="Jane"/>
                  {/* <label className="text-xs text-red-500">Email is required</label> */}
                </div>

                <div className="w-full mt-4 sm:mt-0">
                  <label className="text-xs text-[#464646]">Last Name</label>
                  <br />
                  <input autoComplete="off" className="border-2 focus:outline-[#056424] focus:bg-white text-[#464646] text-sm w-full mt-1 h-12 rounded-md px-2 placeholder:text-xs" type="text" placeholder="Doe"/>
                  {/* <label className="text-xs text-red-500">Email is required</label> */}
                </div>
              </div>
              ...
              <div className="mt-4">
                <label className="text-xs text-[#464646]">Phone Number</label>
                <IntlTelInput
                  placeholder="Enter phone number"
                  preferredCountries={['ng']}
                  containerClassName="intl-tel-input mt-1 w-full"
                  inputClassName="form-control border-2 focus:outline-[#056424] focus:bg-white text-[#464646] text-sm w-full h-12 rounded-md px-2 placeholder:text-xs"
                />
              </div>

              <div className="mt-4">
                <label class="text-xs block mb-1 text-[#464646]" for="password">
                  Password
                </label>
              ...
              <div className="mt-4">
                <label class="text-xs block mb-1 text-[#464646]" for="confirmPassword">
                  Confirm Password
                </label>
                <div class="relative w-full">
                  <div class="absolute inset-y-0 right-0 flex items-center px-2">
                    <input autoComplete="off" class="hidden js-password-toggle" id="toggle" type="checkbox" />
                    <button type="button" onClick={toggleConfirmPassword} class="text-xs px-3 text-[#464646] cursor-pointer js-password-label" for="toggle">{ confirmPasswordType === "password" ? "show" : "hide" }</button>
                  </div>
                  <input type={confirmPasswordType} onChange={handleConfirmPasswordChange} value={confirmPasswordInput} name="password" autoComplete="off" class="appearance-none focus:bg-white border-2 rounded-md w-full text-[#464646] h-12 px-3 leading-tight focus:outline-[#056424] text-sm placeholder:text-xs pr-16 js-password" id="confirmPassword" placeholder="Confirm your password" />
                </div>
                {/* <label className="text-xs text-red-500">Password is required</label> */}
              </div>
